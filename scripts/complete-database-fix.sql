-- Complete Database Fix for One Estela Place
...
SET @sql = (SELECT IF(
  ...
  'SELECT "check_in_date column already exists"'
));
...
SET @sql = (SELECT IF(
  ...
  'SELECT "check_out_date column already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "number_of_guests column already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "available_rooms column already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
  ...
  'SELECT "image_360_url column already exists"'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
-- 6. Update existing data to use new columns
...
-- 7. Insert initial homepage content if table is empty
...
-- 8. Ensure admin users exist with correct passwords
...
-- 9. Ensure initial venue and office rooms exist
...
SELECT 'Complete database fix completed successfully!' as status;